...
class ExerciseNameViewModel: ObservableObject{
    let name: String
    
    var searchExerciseByName = "https://exerciseapi3.p.rapidapi.com/search/?name="
    let headers = [
    ...
    func fetchExercises(){
        searchExerciseByName += "\(name.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed)!)"
        
        guard let url = URL(string: searchExerciseByName) else{
            hasError = true
...
extension ExerciseNameViewModel{
